...
import FloweGarden from '../assets/images/Gemini_Generated_Image_94x13v94x13v94x1-tinypng.io.webp';

export default function GrasseFarmsModal({ isOpen, onClose, onExploreCollection }) {
  const { brandContent, addFarmBooking } = useAdminStore();
  const grasse = brandContent?.grasse || {};
  ...
  const handleBookingSubmit = (e) => {
    ...
    if (typeof addFarmBooking === 'function') {
      addFarmBooking({
        name: visitorName.trim(),
        phone: visitorPhone.trim(),
        date: new Date().toLocaleDateString('fa-IR'),
        status: 'در حال هماهنگی تشریفات',
      });
    }
    setIsSubmitted(true);
  };
            ...
            <div className="grasse-hero">
              ...
              <div className="grasse-hero-content">
                <div className="grasse-location-badge">
                  <MapPin size={14} />
                  <span>{grasse.locationBadge || "پرووانس، فرانسه • Grasse, Côte d'Azur"}</span>
                </div>
                <h2 className="grasse-title font-brand-story">
                  {grasse.title || 'مزارع اختصاصی گل گراس؛ مهد عطرآفرینی جهان'}
                </h2>
                <p className="grasse-lead">
                  {grasse.lead || 'جایی که جادوی خاک پرووانس و نسیم مدیترانه با دستان کهنه‌کار گل‌چینان پیوند می‌خورد تا گران‌بهاترین قطرات عطر جهان متولد شوند.'}
                </p>
              </div>
            </div>
              ...
              <div className="grasse-story-grid">
                <div className="grasse-story-text">
                  ...
                  <p className="grasse-paragraph">
                    {grasse.description || 'شهر تاریخی گراس واقع در جنوب فرانسه، به سبب پناه گرفتن میان قله‌های آلپ و گرمای ملایم دریای مدیترانه، دارای یک ریزاقلیم منحصربه‌فرد در سراسر کره زمین است. شب‌های خنک و روزهای آفتابی، غلظت اسانس روغنی گل‌ها را به اوج شکوفایی می‌رساند.'}
                  </p>
                  <p className="grasse-paragraph">
                    خانه عطر آنتی دارای {grasse.farmStat1Num || '۵ هکتار باغ اختصاصی'} است که منحصراً با کودهای زیستی غنی و آب چشمه‌های طبیعی سیراب می‌شوند؛ بدون ذره‌ای آفت‌کش شیمیایی.
                  </p>
                </div>
                ...
                <div className="grasse-highlights-card">
                  ...
                  <div className="highlight-item">
                    ...
                    <div>
                      <strong>{grasse.farmStat2Num || '۴۰۰ کیلوگرم گل رز'}</strong>
